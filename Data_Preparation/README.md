...
## MMseq2
For our clusterisation procedure the MMseqs2 software (version) was used. MMseqs2 is an open-source software use to cluster large databases of sequences. input file were: negative_dataset.tsv and positive_dataset.tsv. For each runtwo output files were created. For the negative set: 
- *neg_cluster.tsv*: TSV containing two colums ( ID of each sequence in th einput file, ID of the representative sequence indetifying the cluster). 
- *neg_rep_seq.fasta*: .fasta file containing all the representative sequences, one for each found cluster. 
  The same output ( positive_cluster.tsv, positive_rep_seq.fasta ) was obtained using as *input positive_dataset.tsv*
